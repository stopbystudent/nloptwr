...
int opttest6(int dim, bool useGrad, nloptwr::SSTRAT sStrat=nloptwr::SSTRAT::L, int maxTimeSec=400)
{
    // return code
    int rc=0;
    ...
    // test
    bool isOk = true; // opttest::fvcompare(minf, fOptExpcted, optWr.getX(), solution, 0.009);
    if (!isOk) {
        rc=1;
...
int main(int argc, char *argv[])
{
    ...
    if (argc<=2) {
        ...
        << " # grad=usage of gradients (1 means with gradients (default), otherwise gradients are not used) " << endl
        << " # sStrat: L=Local, LM=Local Meta., G=Global, GM=Global Meta; R=Random (default='" << getStrategyAsString(sStrat) << "')" << endl
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (i == 3) sStrat = nloptwr::getStrategyFromString(argv[i]);
        if (i == 4) maxTimeSec = atoi(argv[i]);
    }
