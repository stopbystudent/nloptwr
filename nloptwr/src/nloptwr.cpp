...
NLOptWrapper::NLOptWrapper (
  ...
  :
  optFknBases(),
  nDim ( static_cast<long int>(oFktn.getSizeOfX()) ),
  mDim ( static_cast<long int>(oFktn.getSizeOfNeConstraints()) ),
  opt ( nullptr ),
  x ( oFktn.getXInitial() ),
  ub ( oFktn.getUpperBounds() ),
  lb ( oFktn.getLowerBounds() ),
  nThreads (static_cast<size_t>( ( nThr>0 ) ? nThr : utl::OmpHelper::getNumProcs() ) ),
  dX ( nDim, 1.0E-8 )

{
  ...
  */
  ...
  if ( optFknBases.size() !=static_cast<size_t>(nThreads) ) optFknBases.resize (static_cast<size_t>(nThreads) );
...
{
  for (long int i=0; i<nDim; i++ ) dX[i] = val;
}


void NLOptWrapper::setDx ( const std::vector<double>& vals )
{
  for (long int i=0; i<nDim && i<static_cast<long int>(vals.size()); i++ ) dX[i] = vals[i];
}
...
bool NLOptWrapper::checkVectors() const
{
  ...
  if ( nDim != static_cast<long int>(lb.size()) ) result=false;
  if ( nDim != static_cast<long int>(ub.size()) ) result=false;
  if ( nDim != static_cast<long int>(xTolAbs.size()) ) result=false;
  if ( nDim != static_cast<long int>(initial_step.size()) ) result=false;
  if ( nDim != static_cast<long int>(initialSubStep.size()) ) result=false;
...
// ------------------------------------------------------------------------------------------------
void NLOptWrapper::setSearchMin(bool sMin) {
    searchMin=sMin;
}
...
*/
bool NLOptWrapper::getSearchMin() const {
    return searchMin;
}
...
)
{
  ...
  cGrad.resize ( mDim );
  for (long int i=0; i<mDim; i++ ) cGrad[i].resize ( nDim );
  ...
  // TODO:
  if ( alg1.isSetPopulation() ) opt->set_population ( population );
  ...
  if ( algs.size() >=2 )
    ...
    sub_opt.set_ftol_abs(getFTolAbsSubOpt());
      ...
      if ( alg2.isSetPopulation() ) sub_opt.set_population ( population );
...
unsigned int NLOptWrapper::getPopulation() const {
    return population;
}
